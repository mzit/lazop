...
class LazopClient
{
    ...
    /**
     * LazopClient constructor.
     * @param string $url
     * @param string $appKey
     * @param string $secretKey
     * @param string $logPath log文件路径+文件名
     * @throws Exception
     */
    public function __construct($url = "", $appKey = "", $secretKey = "",$logPath = "")
    {
        $length = strlen($url);
        if ($length == 0) {
     ...
     * get请求
     * @param $url
     * @param null $apiFields
     * @param null $headerFields
     * @return bool|string
     * @throws Exception
     */
...
//        $logger->conf["log_file"] = rtrim(LAZOP_SDK_WORK_DIR, '\\/') . '/' . "logs/lazopsdk.log." . date("Y-m-d");
        $logger->conf["log_file"] = $this->logPath;
